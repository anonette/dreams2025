...
class TemporalDispersionManager:
    ...
    def get_next_call_delay(self) -> float:
        ...
        # If temporal dispersion is disabled, return minimal delay for API rate limiting only
        if not self.config.use_temporal_dispersion:
            return 0.1  # Minimal delay for API rate limiting
...
class BatchDreamGenerator:
    ...
    async def generate_dream_with_protocol(self, language: str, batch_id: str, 
        ...
        generation_config = GenerationConfig(
            ...
            max_tokens=1000,  # Target 1000 tokens for detailed dreams
        ...
        try:
            # Make API call with enhanced protocol including system message
            system_message = config.get('system_message')
            dream = await self.llm_interface.generate_dream(modified_prompt, generation_config, system_message)
            end_time = time.time()
    ...
    async def _save_session_logs(self):
        ...
        temporal_filename = f"{self.base_logs_dir}/temporal_statistics_{self.session_id}.json"
        with open(temporal_filename, 'w', encoding='utf-8') as f:
            json.dump({
                'session_id': self.session_id,
                'timestamp': timestamp,
