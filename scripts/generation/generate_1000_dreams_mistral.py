#!/usr/bin/env python3
"""
Generate 1000 dreams per language using Mistral Nemo via OpenRouter
Uses the same prompt setup: "You woke up and immediately wrote down your long dream. What did you write?"
"""
...
# Import the optimized configuration and LLM interface
from optimized_dream_languages import LANGUAGE_CONFIG
from src.models.llm_interface import LLMInterface, GenerationConfig

@dataclass
class MistralDreamConfig:
    """Configuration for Mistral Nemo dream generation"""
    version: str = "1.0"
    system_name: str = "MistralNemoDreamGenerator"
    session_prefix: str = "MISTRAL_"
    model: str = "mistralai/mistral-nemo"  # OpenRouter model identifier
    ...
    scenario_type: str = "Pure Immediate Dream Writing via Mistral Nemo"

class MistralDreamGenerator:
    """Dream generator using Mistral Nemo via OpenRouter"""
    
    def __init__(self, api_keys: Dict[str, str]):
        self.config = MistralDreamConfig()
        ...
        self.base_logs_dir = 'logs_mistral'
        ...
        print(f"🚀 Mistral Nemo Dream Generator v{self.config.version}")
    ...
    def setup_language_directory(self, language: str) -> str:
        ...
        lang_dir = f"{self.base_logs_dir}/{language}/mistral-nemo"
    ...
    async def generate_single_dream(self, language: str, dream_number: int) -> Dict[str, Any]:
        """Generate a single dream using Mistral Nemo"""
        
        config = LANGUAGE_CONFIG[language]
        prompt = config['prompt']  # Same prompt as optimized config
        
        # Generate unique IDs
        call_id = str(uuid.uuid4())
        batch_id = f"mistral_batch_{language}_{datetime.now().strftime('%H%M%S')}"
        user_id = f"mistral_user_{uuid.uuid4().hex[:8]}_{np.random.randint(10000, 99999)}"
        prompt_id = f"mistral_{hash(prompt) % 100000000:08x}"
        
        start_time = time.time()
        
        # Generation config for Mistral Nemo
    ...
    async def generate_all_languages(self) -> Dict[str, Any]:
        ...
        print(f"🚀 MISTRAL NEMO DREAM GENERATION")
    ...
    async def save_global_summary(self, results: Dict, total_successful: int):
        ...
        summary_data = {
            ...
            'results': {
                ...
                'avg_chars_per_dream': np.mean([len(d['dream']) for d in all_dreams if d.get('status') == 'success']),
                'avg_words_per_dream': np.mean([d['word_count'] for d in all_dreams if d.get('status') == 'success'])
            },
            'configuration': {
                'model': self.config.model,
                'temperature': self.config.temperature,
...
async def generate_1000_mistral_dreams():
    """Generate 1000 dreams per language using Mistral Nemo"""
    
    print("🎯 GENERATING 1000 DREAMS PER LANGUAGE WITH MISTRAL NEMO")
    print("=" * 50)
    print("🤖 Model: mistralai/mistral-nemo (via OpenRouter)")
    print("📊 Target: 1000 dreams per language (5000 total)")
    print("📝 Prompt: Same as optimized config")
    ...
    # Create generator
    generator = MistralDreamGenerator(api_keys)
...
if __name__ == "__main__":
    asyncio.run(generate_1000_mistral_dreams()) 