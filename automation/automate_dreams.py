...
class DreamsAutomation:
    ...
    def generate_research_report(self) -> bool:
        ...
        try:
            ...
            # Check if we have API keys for typological analysis
            has_api_keys = any(os.getenv(key) for key in ['OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'OPENROUTER_API_KEY'])
            ...
            # Add typological analysis if we have API keys
            if has_api_keys:
                cmd.extend(["--max-dreams-per-language", "30"])  # Use moderate number for automation
                self.logger.info("Including typological linguistic analysis (API keys available)")
            else:
                cmd.append("--no-typological-analysis")
                self.logger.info("Skipping typological analysis (no API keys)")
            
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=600)  # Increase timeout for typological analysis
